...
if __name__=='__main__':
    ...
    while True:
        ...
        if not nic.isconnected():
            LCD.text("disconnected...",10,30,LCD.white)
            LCD.show()
            time.sleep(1)
        else: # connected
            ip,netmask,gateway,dns = nic.ifconfig()
    
            LCD.text("IP addr: " + ip,10,30,LCD.white)
    
            LCD.text("Netmask: " + netmask,10,50,LCD.white)
            LCD.text("Gateway: " + gateway,10,70,LCD.white)
            LCD.text("DNS    : " + dns,10,110,LCD.white)
        
        
            online = pingdns('8.8.8.8')
            if online == True:
                LCD.text("Internet : ONLINE",10,150,LCD.green)
            else:
                LCD.text("Internet : Offline",10,150,LCD.red)
            LCD.show()
